...
Required env vars: `PORT` (api-server and cve-dashboard, no default — fails fast if unset), `BASE_PATH` (cve-dashboard vite config, no default). Optional: `NVD_API` (api-server) — an NVD API key, sent as the `apiKey` header on all NVD requests; raises the rate limit from 1 req/6s to 50 req/30s and lets the weekly-fetch severity buckets run concurrently instead of staggered. `DATABASE_URL` (api-server) — optional Postgres connection string; enables the persistent CVE snapshot cache and change history (see API server architecture below). `@workspace/db`'s `db`/`pool` exports are `null` when unset, and every consumer degrades gracefully — api-server runs exactly as before with no DB configured. `APPLE_VULN_API_URL` / `APPLE_VULN_API_KEY` (api-server) — base URL and `X-API-Key` for the separate `ios-security-vulnerability-formatter` service (own repo/Railway project); powers `GET /apple/patches` (`routes/apple-patches.ts`). Missing either var makes that endpoint fail with a 502 — the rest of api-server is unaffected.
...
- `lib/db` — Drizzle ORM + `pg`, Postgres. Schema (`src/schema/index.ts`): `cveSnapshots` (current-state upsert per CVE) and `cveChanges` (append-only log of tracked-field flips). `db`/`pool` are `null` when `DATABASE_URL` is unset — always null-check before use; this is a deliberately optional dependency, not a guaranteed one. `api-server` is the only consumer, via `src/lib/cve-store.ts`.
...
Everything in `artifacts/api-server/src/routes/*.ts` is a **live aggregator with an in-memory cache**, not a database-backed CRUD API — Postgres (when configured) is an optional durability/history sidecar for that cache, not the source of truth (NVD/CISA/MSRC still are):
...
- `warmWeeklyCache()` runs once, fire-and-forget, right after the server starts listening (`index.ts`) so the first real request doesn't pay the cold-fetch cost; if Postgres is configured, it first warms the in-memory cache from the last-persisted snapshot (`cve-store.ts`'s `loadCveSnapshots()`) so this is near-instant even before the live NVD refresh finishes. `index.ts` also calls the exported `refreshCveCache()` on a recurring `setInterval` (61 min — just past the weekly cache's 60-min TTL) so the cache is refreshed proactively instead of only reactively whenever a request happens to land on an expired one.
- `fetchDailyCves` (`/cves/daily`, `/cves/summary`) has no NVD call of its own — it's derived by filtering `fetchWeeklyCves`'s already-fetched 7-day dataset down to the last 24h, so it automatically inherits the same warm-start/persistence/scheduled-refresh behavior as weekly instead of needing its own.
- After each weekly fetch, `saveCveSnapshots()` upserts the results into `cveSnapshots` (fire-and-forget — never blocks the response) and logs any tracked-field change (severity/CVSS/patch-status/known-exploited) into `cveChanges`, surfaced via `GET /cves/changes`.
...
- `cve-dashboard`'s `VITE_SITE_URL` build arg is the absolute origin the deployment is publicly reachable at (e.g. `https://daily-patch-report.staging.labm.dev`, no trailing slash) — baked into `index.html`'s `og:image`/`twitter:image`/`og:url` tags at build time, since those protocols require absolute URLs. Differs per environment (staging vs. production), so it must be set as a **build** variable on each Railway service individually, same mechanism as `VITE_API_URL`. Unset (e.g. local `vite build`), Vite leaves the literal `%VITE_SITE_URL%` placeholder in place with a build warning — harmless for local dev, just don't ship a build like that.
