...
- `warmWeeklyCache()` runs once, fire-and-forget, right after the server starts listening (`index.ts`) so the first real request doesn't pay the cold-fetch cost; if Postgres is configured, it first warms the in-memory cache from the last-persisted snapshot (`cve-store.ts`'s `loadCveSnapshots()`) so this is near-instant even before the live NVD refresh finishes.
